...
reanalyze the RV data and perform dynamic analysis on several of these
...
HRS. HET will become the second telescope, besides Keck, capable of RV
follow-up on planet candidates discovered by \kepler. This will also
benefit other planet search programs on \hrs\ such as surveys on
long-period planets and multiple-planet systems.
...
spectrograph. It will perform dedicated RV monitoring on a
carefully-selected ensemble of nearby stars. It is expected to
discover $\gtrsim 10$ Earth- to super-Earth-size planets with orbital
period of 1--100 days around nearby stars, with 3--5 expected to be in
...
not carefully studied and corrected for.
...
\begin{wrapfigure}{r}{0.48\textwidth}
  \vspace{-25pt}
  \begin{center}
    \includegraphics[width=0.45\textwidth]{vank}
  \end{center}
  \vspace{-25pt}  
...
preparation for its upgrade (schedule to finish in 2014 summer) and
also for the upcoming \minerva. The new \hrs\ will have a factor of
$\sim$5 improvement in throughput, with a new spectral format that is
similar to CHIRON and \minerva's. We have also acquired on-sky and
engineering test data in order to address the issue of modeling this
fiber-fed spectrograph. Since we have set up the data reduction and
Doppler pipeline for the current \hrs\ \citep{wang2012}, we are
confident about upgrading the pipeline for the new \hrs\ with the aim
to achieve $\sim$1~\mps\ precision.


\begin{comment}
The main objective of this proposal is to develop data reduction and
RV extraction pipelines for HET/HRS and Minerva. The similarity
between the spectral data products of the upgraded HET/HRS and Minerva
provides us the opportunity to construct a single pipeline that will
work equally well for both. The RV precision of the current HET/HRS is
3-5 m/s, and we plan to improve it towards 1m/s through a better
understanding of the spectrograph, a better modeling of the
instrumental profile and so on. Such an improvement will enable more
discoveries of long-period planet with HRS archival RV data spanning
~10 years, and it will also prepare us to bring the upgraded HET/HRS
to ~1 m/s precision or even better as it takes in new optical parts
and arrangements and adopts a new spectral data format. 
